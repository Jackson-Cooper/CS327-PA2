...
public class CooperJacksonHarmonCameronCRT {
    public static BigInteger[] encryptWithPrint() {
        ...
        BigInteger mp = ciphertext.modPow(dp, p);
        BigInteger mq = ciphertext.modPow(dq, q);

        BigInteger a = mp.multiply(q).multiply(q.modInverse(p)).add(mq.multiply(p).multiply(p.modInverse(q)));

        // decrypt ciphertext
        BigInteger plaintext = a.mod(N);
        ...
        System.out.println("m2 = " + plaintext.toString(16));

        return arr;
    }

    public static void decryption(BigInteger p, BigInteger q, BigInteger N, BigInteger d, BigInteger ciphertext) {
        // CRT decryption process
        BigInteger dp = d.mod((p.subtract(BigInteger.ONE)));
        BigInteger dq = d.mod((q.subtract(BigInteger.ONE)));
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 1000; i++) {
            decryption(dN[0], dN[1], dN[2], dN[3], dN[4]);
        }
        ...
        System.out.printf("CRT Decryptions took %.0f milliseconds", elapsedTime);
        System.out.println();
        System.out.printf("CRT Decryptions took %.3f kilobits/second ", kbps);
        System.out.println();
